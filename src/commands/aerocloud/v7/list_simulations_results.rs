use crate::{
    ...
    queries::aerocloud::{
        ProjectV7WithSimulationsResults, SimulationQuality,
        SimulationsWithResultsInProjectV7Arguments,
        SimulationsWithResultsInProjectV7Query, Speed, YawAngle,
    },
};
...
pub async fn run(
    ...
    let op_args = SimulationsWithResultsInProjectV7Arguments {
    ...
    let op = SimulationsWithResultsInProjectV7Query::build(op_args);
...
fn print_human(project: &ProjectV7WithSimulationsResults) {
        ...
        if let Some(results) = &sim.results {
                ...
                results.yaw_angles.as_ref().unwrap_or(&Vec::new())
            {
                table.add_row(vec![
                    ...
                    format!("{}", yaw_angle_results.surface),
                    ...
                    format!("{}", yaw_angle_results.heat_transfer.value,),
                    format!("{}", yaw_angle_results.heat_transfer.coefficient,),
                ]);
            }
        }
    }
