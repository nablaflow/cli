...
pub mod set_ws_hostname;
pub mod show;
pub mod unset_hostname;
pub mod unset_ws_hostname;
...
pub async fn run(
    ...
    match subcommand {
        ...
        ConfigScope::SetWsHostname { hostname } => {
            self::set_ws_hostname::run(args, config, hostname).await
        }
        ConfigScope::UnsetHostname => {
            self::unset_hostname::run(args, config).await
        }
        ConfigScope::UnsetWsHostname => {
            self::unset_ws_hostname::run(args, config).await
        }
